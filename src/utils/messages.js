// ============================================
// DOGUINHA STORE BOT - GERENCIADOR DE MENSAGENS
// ============================================
...
function getMessage(key) {
    try {
        ...
        logger.warning(`Mensagem "${key}" não encontrada no banco`);
        return `⚠️ Mensagem não configurada: ${key}`;
...
function processMessageVariables(key, userData = {}) {
    ...
    const variables = {
        ...
        '{support_telegram_link}': userData.support_telegram_link || '',
        '{telegram_bot_link}': userData.telegram_bot_link || '',
        '{telegram_group_link}': userData.telegram_group_link || '',
    };
